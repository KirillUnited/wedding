...
function UpdateLoginForms() {
    $('#account-desktop-block').load('/Account/_AccountDeskTop');
    $('#account-block').load('/Account/_AccountMobile');
    $('#signupcontainer').load('/Account/_LoginRegistrationModal?immediatelyShow=false');
}

function UpdateCurrentUserData() {
    //update current user data
    $.post({
        url: "/UserDetails/GetCurrentUserData",
        dataType: 'json',
        success: function (data) {
            if (data.Result) {
                ...
                if (data.IsAuth) {
                    ...
                    $("input[name='Email']").val(data.UserEmail);
                    $("input[name='WeddingDate']").val(data.WeddingDate);
                }
            }
        }
    });
